const AUTH_CONFIG = {
    EXTERNAL_AUTH_BASE_URL: "https://platform-development-dev.157.20.214.214.nip.io/auth/api",
    get VERIFY_OTP_URL() { return `${this.EXTERNAL_AUTH_BASE_URL}/auth/verify-otp`; },
    get LOGIN_URL() { return `${this.EXTERNAL_AUTH_BASE_URL}/auth/login`; },
    get CLIENTS_URL() { return `${this.EXTERNAL_AUTH_BASE_URL}/clients`; }
};
...
class APIClient {
    ...
    async getAuthToken(identifier, password) {
        // Dynamically choose endpoint based on input type
        const isEmail = identifier.includes('@');
        const authUrl = isEmail ? AUTH_CONFIG.LOGIN_URL : AUTH_CONFIG.VERIFY_OTP_URL;

        const payload = isEmail
            ? { email: identifier, password: password }
            : { phone: identifier, password: password, captchaToken: null };
    ...
    async getAvailableOrganizations(token, keyword = "") {
        ...
        try {
            do {
                const clientsUrl = `${AUTH_CONFIG.CLIENTS_URL}?page=${currentPage}&limit=${limit}${kParam}`;
                console.log(`Fetching organizations page ${currentPage} (keyword: "${keyword}")...`);
                ...
                if (Array.isArray(data)) {
                    ...
                    totalPages = data.total_pages || (data.meta ? data.meta.total_pages : 1) || data.last_page || 1;

                    if (!data.total_pages && data.total && (data.limit || limit)) {
                        totalPages = Math.ceil(data.total / (data.limit || limit));
                    }

                    const hasNext = !!(data.next || (data.links && data.links.next) || (data.meta && data.meta.next_page) || (data.pagination && data.pagination.next_page));
                    if (hasNext && totalPages <= currentPage) {
                        totalPages = currentPage + 1;
                    }
                }

                console.log(`Page ${currentPage} returned ${clients.length} clients. Total found so far: ${allClients.length + clients.length}`);
        ...
        const findName = (obj) => {
            ...
            const highPriorityKeys = ['name', 'orgn_name', 'organization_name', 'display_name'];
            for (const key of highPriorityKeys) {
                if (obj[key] && typeof obj[key] === 'string' && obj[key].trim() !== "" && obj[key].toLowerCase() !== "generic") {
                    return obj[key];
                }
            }
            ...
            for (const key in obj) {
                if (typeof key === 'string' && key.toLowerCase().includes('name') && typeof obj[key] === 'string') {
                    return obj[key];
                }
            }
            return null;
    ...
    async login(mobile_number, password, clientId = null) {
        const authData = await this.getAuthToken(mobile_number, password);
